...
from agents.enums.enums import ToolType  # Fixed import
...
class Tools(Base):
    ...
    tool_type: Mapped[ToolType] = mapped_column(Enum(ToolType), nullable=False)  # Fixed enum
    ...
    skills: Mapped[list["Skill"]] = relationship("Skill", secondary="skill_tools", back_populates="skills")

    @validates('command', 'args', 'url', 'headers')
    def validate_tool(self, key, value):
        if key == 'command' and self.tool_type == ToolType.LOCAL and not value:
            raise ValueError("Command is required for LOCAL tool type")
        if key == 'url' and self.tool_type in [ToolType.HTTP, ToolType.SSE] and not value:
